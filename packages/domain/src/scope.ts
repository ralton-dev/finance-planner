...
/**
 * The same pass, with every intermediate it stepped through recorded.
 *
 * The collector is inert when absent, so `computeScopePlan` above and this are
 * the same arithmetic — a trace can never be a second engine. What comes back
 * is structured, not prose: laying it out as text is the API's job
 * (`plan-debug-report.ts`), because it needs nothing this does not publish.
 */
